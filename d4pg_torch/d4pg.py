...
class LearnerD4PG(object):
    ...
    def __init__(
        self, config, policy_net, target_policy_net, learner_w_queue, log_dir=""
    ):
    ...
    def _update_step(self, batch, replay_priority_queue, update_step):
        ...
        policy_loss = policy_loss * torch.from_numpy(self.value_net.z_atoms).float().to(
            self.device
        )
        ...
        if update_step.value % 100 == 0:
            try:
                params = [
                    p.data.cpu().detach().numpy() for p in self.policy_net.parameters()
                ]
        ...
        self.logger.scalar_summary(
            "learner/learner_update_timing", time.time() - update_time, step
        )
